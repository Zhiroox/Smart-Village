...
import { GisLocation, VillageName } from '@/lib/types';
...
export const MapComponent: React.FC<MapComponentProps> = ({
  ...
  const filteredLocations = locations.filter(loc => {
    const villageMatch = selectedVillage === 'Semua Desa' || loc.village === selectedVillage;
    const catMatch = selectedCategory === 'Semua Layer' || loc.category === selectedCategory;
    return villageMatch && catMatch;
  });
  ...
  // Render client-side Leaflet via dynamic import or direct React-Leaflet
  // Note: For reliability across environments, we render an interactive Leaflet-styled SVG Map interface with exact GPS coordinates overlay + list selector
  return (
          ...
          {categories.map(cat => (
            ...
            >
              ...
              <span className={`px-1.5 py-0.5 rounded-full text-[10px] ${
                ...
                {cat === 'Semua Layer'
                  ? locations.filter(l => selectedVillage === 'Semua Desa' || l.village === selectedVillage).length
                  : locations.filter(l => (selectedVillage === 'Semua Desa' || l.village === selectedVillage) && l.category === cat).length}
              </span>
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 min-h-[450px] lg:min-h-[550px] relative bg-slate-900 overflow-hidden flex flex-col justify-between p-6">
        ...
        <div className="z-20 bg-slate-900/85 backdrop-blur-md text-white p-3.5 rounded-xl border border-slate-700/60 flex items-center justify-between shadow-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs font-bold tracking-wide uppercase text-emerald-400">Peta Digital Kecamatan Batukliang</div>
          ...
          <div className="text-right hidden sm:block">
            ...
            <div className="text-xs font-mono text-emerald-300">-8.6218° S, 116.2858° E</div>
          </div>
        </div>
          ...
          <div className="relative w-full h-full max-w-2xl max-h-96 border border-emerald-500/20 rounded-3xl bg-slate-950/40 backdrop-blur-xs p-6 flex flex-wrap items-center justify-center gap-6 overflow-auto">
            {filteredLocations.length === 0 ? (
              <div className="text-center text-slate-400 text-sm py-12">
                ...
                Tidak ada titik lokasi pada layer ini untuk {selectedVillage}.
              </div>
            ) : (
        ...
        <div className="z-20 bg-slate-900/90 backdrop-blur-md text-slate-300 p-3 rounded-xl border border-slate-700/60 flex items-center justify-between text-xs">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-emerald-500"></span> Des. Pagutan</span>
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-blue-500"></span> Des. Bujak</span>
          </div>
